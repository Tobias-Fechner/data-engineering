...
songplay_table_create = ("""
    CREATE TABLE IF NOT EXISTS songplays (songplay_id SERIAL, start_time timestamp, user_id varchar, level varchar, song_id varchar, artist_id int, session_id varchar, location varchar, user_agent varchar);
""")
...
song_select = ("""
    SELECT songs.song_id, artists.artist_id FROM songs JOIN artists ON songs.artist_id=artists.artist_id WHERE songs.title=%(title)s AND artists.name=%(name)s AND songs.duration=%(duration)s;
""")
# QUERY LISTS
